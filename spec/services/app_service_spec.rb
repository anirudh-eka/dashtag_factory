    ...
    context "when heroku app setup creation is successful" do 
      ...
      context "when polling status results in a failed" do 
        it "should return hash with status failed and error message" do 
          response_failure = {"id"=>"c3cd21ed-44b8-4cb9-8d56-c0781cee637c", 
          ...
          allow(heroku_app_setup_mock).to receive(:info).with(app_setup_id).and_return(response_failure)

          result = app_service.create(client_mock, request_body)
          expect(result).to eq({"status"=> "failed", "message" => "Heroku deploy failed."})
        end 
      end
    end
  end
